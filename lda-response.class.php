...
class LinkedDataApiResponse {
	...
	function addMetadataToPage(){
		...
		if(!empty($metadataRequested[0])){
			...
			$this->DataGraph->add_resource_triple($this->pageUri, API.'extendedMetadataVersion', $this->Request->getUriWithParam('_metadata', 'all,views,formats,execution,bindings'));
		}
	}
	...
	function addFormattersMetadata(){
		...
		foreach($this->ConfigGraph->getFormatters() as $formatName => $formatUri){
          ...
          if($this->pageUri!==$altFormatUri){
              ...
              $mimetype = $this->ConfigGraph->getMimetypesOfFormatterByName($formatName);
              $this->DataGraph->add_literal_triple('_:format_'.$formatName, RDFS_LABEL, $mimetype[0]);
        }
      }
	}
	...
	function addExecutionMetadata(){
		...
		if(strpos($endpointUrl, '_:')===0) $endpointUrl = CONFIG_URL;
		 ...
		 $this->DataGraph->add_resource_triple('_:execution', API.'endpoint', $this->endpointUrl);
